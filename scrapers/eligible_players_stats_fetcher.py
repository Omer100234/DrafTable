...
if __name__ == "__main__":
    # Get player stats per conference (4 API calls)
    for conf, filename in CONF_FILE_NAMES.items():
        data = fetch_stats_by_conference(2025, conf)
        if data:
            save(pd.DataFrame(data), filename)

    # Notre Dame stats (1 API call)
    nd_data = fetch_stats_by_team(2025, "Notre Dame")
    if nd_data:
        save(pd.DataFrame(nd_data), "notre_dame_players_stats.csv")

    logger.info("Done! Total API calls used: 5")
