"""
Normalize tight end variables for ML.

Same strategy as WR normalizer:
- Arctan (s=8): forty (center=4.65), cone (center=7.10), shuttle (center=4.40)
  Centers shifted for TE body type (slower than WRs).
...
PARAMS_PATH = os.path.join(os.path.dirname(__file__), "norm_params_tight_end.json")
...
# TE-specific arctan centers (TEs are bigger/slower than WRs)
ARCTAN_COLS = {
    "forty": (4.65, 8),
    "cone": (7.10, 8),
    "shuttle": (4.40, 8),
...
# Drop LONG and fumbles (no returns for TEs)
...
if __name__ == "__main__":
    ...
    train_path = os.path.join(PROCESSED_DIR, "variables_tight_end.csv")
    train_df = pd.read_csv(train_path)
    logger.info(f"Loaded {len(train_df)} training TEs, {len(train_df.columns)} columns")

    train_df = preprocess(train_df)
    normalized = fit_normalize(train_df)

    outpath = os.path.join(PROCESSED_DIR, "normalized_tight_end.csv")
    ...
    for year in range(2023, 2027):
        filepath = os.path.join(PROCESSED_DIR, f"variables_class_{year}_tight_end.csv")
        ...
        pred_outpath = os.path.join(PROCESSED_DIR, f"normalized_class_{year}_tight_end.csv")
        pred_norm.to_csv(pred_outpath, index=False)
        logger.info(f"Saved {year}: {len(pred_norm)} TEs -> {pred_outpath}")
